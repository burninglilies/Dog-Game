	...
	public class PathMagicEditor : Editor
	{
		...
		public override void OnInspectorGUI ()
		{
			...
			if (currentSelectedWaypoint != -1) {
				...
				if (waypointsFoldout) {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.PropertyField (waypoints.GetArrayElementAtIndex (currentSelectedWaypoint).FindPropertyRelative ("id"), new GUIContent ("ID", "String name of waypoint"));
                    EditorGUIUtility.labelWidth = 60;
                    EditorGUILayout.EndHorizontal(); 
		...
		{ 
			...
			for (int i = 0; i < waypoints.arraySize; i++) {
				...
				if (!pm.FindProperty ("updateTransform").boolValue || pm.FindProperty ("target").objectReferenceValue == null) {
                    ...
                    Handles.DotHandleCap (0, pos, Quaternion.identity, handleSize / 10f, EventType.Layout);   
					Handles.color = Color.white;
				}
			}
			...
			if (pmo.presampledPath) {
				...
				if (showPathSamples) {
					...
					for (int i = 1; i < pmo.samplesNum; i++) {
                        ...
                        Handles.DotHandleCap (0, pmo.positionSamples [i - 1], Quaternion.identity, handleSize / 20f, EventType.Layout);
					}
				}
			}
		...
		void DrawRotationsForSegment (SerializedObject pm, int index)
		{
			...
			for (int j = 0; j < 10; j++) {
                ...
                Handles.ArrowHandleCap (0, pos, rot, capSize, EventType.Layout);
				Handles.color = Color.white;
			}
		}
